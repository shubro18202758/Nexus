...
import { boolean, customType, jsonb, pgEnum, pgTable, text, timestamp, uuid, integer } from "drizzle-orm/pg-core";
...
// ==========================================
// Channel Settings (WhatsApp/Telegram group → category mapping)
// ==========================================

export const channelTypeEnum = pgEnum("channel_type", [
  "academic_official",   // University announcements, prof channels
  "academic_unofficial", // Study groups, peer notes
  "career",              // Internships, hackathons, job boards
  "social",              // Casual chats, memes, noise
  "uncategorized",       // Auto-created channels awaiting user tagging
]);

export const channelSettings = pgTable("channel_settings", {
  id: uuid("id").primaryKey().defaultRandom(),
  sourceId: text("source_id").notNull().unique(), // e.g. WhatsApp group JID or Telegram chat ID
  name: text("name").notNull(),                   // Human-readable group name
  type: channelTypeEnum("type").notNull(),         // Category assigned by user
  platform: text("platform").notNull(),            // 'WhatsApp' | 'Telegram'
  isActive: boolean("is_active").default(true).notNull(), // Pause/resume listening
  createdAt: timestamp("created_at").defaultNow().notNull(),
});

export type ChannelSetting = InferSelectModel<typeof channelSettings>;
export type NewChannelSetting = InferInsertModel<typeof channelSettings>;
...
// Category assigned by The Sentinel (classification agent)
...
export const events = pgTable("events", {
  ...
  // ── New: link to channel_settings for group-aware classification ──
  channelId: uuid("channel_id")
    .references(() => channelSettings.id, { onDelete: "set null" }),
  // ── New: Sentinel-assigned category ──
  category: eventCategoryEnum("category"),
  title: text("title"),
  description: text("description"),
  eventDate: timestamp("event_date"),
  deadline: timestamp("deadline"),       // Clerk-extracted registration deadline
  location: text("location"),            // Clerk-extracted location / "Virtual"
  url: text("url"),
  rawContext: text("raw_context").notNull(),
  metadata: jsonb("metadata"), // { groupName, author, messageId, tags: string[] }
...
// ==========================================
// Event Plans (Strategist output — 1:1 with events)
// ==========================================
...
export const eventPlans = pgTable("event_plans", {
  ...
  // Day-by-day action plan generated by The Strategist
  // Shape: { days: [{ date: string, tasks: [{ title, description, done }] }] }
  generatedPlan: jsonb("generated_plan").notNull(),
  studentFeedback: text("student_feedback"),        // Optional student override / notes
  progress: integer("progress").default(0).notNull(), // 0-100 completion
  isLocked: boolean("is_locked").default(false).notNull(), // Lock after student approves
  createdAt: timestamp("created_at").defaultNow().notNull(),
